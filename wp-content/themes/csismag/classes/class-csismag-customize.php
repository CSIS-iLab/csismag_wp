		...
		public static function register( $wp_customize ) {
			...
			$wp_customize->selective_refresh->add_partial(
				...
				array(
					...
					'render_callback' => 'csismag_customize_partial_site_logo',
				)
			);
			...
			$wp_customize->add_control(
				...
				array(
					...
					'priority'    => 10,
					'label'       => __( 'Retina logo', 'csismag' ),
					'description' => __( 'Scales the logo to half its uploaded size, making it sharp on high-res screens.', 'csismag' ),
				)
			);
			...
			$wp_customize->add_control(
				new WP_Customize_Color_Control(
					...
					'header_footer_background_color',
					array(
						'label'   => __( 'Header &amp; Footer Background Color', 'csismag' ),
						'section' => 'colors',
					)
				)
			);
			...
			$wp_customize->add_control(
				...
				array(
					'type'    => 'radio',
					'section' => 'colors',
					'label'   => __( 'Primary Color', 'csismag' ),
					'choices' => array(
						'default' => __( 'Default', 'csismag' ),
						'custom'  => __( 'Custom', 'csismag' ),
					),
				)
			);
			...
			$wp_customize->add_control(
				new WP_Customize_Color_Control(
					...
					array(
						'section'         => 'colors',
						'settings'        => 'accent_hue',
						'description'     => __( 'Apply a custom color for links, buttons, featured images.', 'csismag' ),
						'mode'            => 'hue',
			...
			$wp_customize->add_section(
				'options',
				array(
					'title'      => __( 'Theme Options', 'csismag' ),
					'priority'   => 40,
					'capability' => 'edit_theme_options',
				)
			);
			...
			$wp_customize->add_control(
				...
				array(
					...
					'section'  => 'options',
					'priority' => 10,
					'label'    => __( 'Show search in header', 'csismag' ),
				)
			);
			...
			$wp_customize->add_control(
				...
				array(
					...
					'section'  => 'options',
					'priority' => 10,
					'label'    => __( 'Show author bio', 'csismag' ),
				)
			);
			...
			$wp_customize->add_control(
				...
				array(
					...
					'section'  => 'options',
					'priority' => 10,
					'label'    => __( 'On archive pages, posts show:', 'csismag' ),
					'choices'  => array(
						'full'    => __( 'Full text', 'csismag' ),
						'summary' => __( 'Summary', 'csismag' ),
					),
				)
			);
			...
			$wp_customize->add_section(
				...
				array(
					'title'       => __( 'Cover Template', 'csismag' ),
					'capability'  => 'edit_theme_options',
					'description' => __( 'Settings for the "Cover Template" page template. Add a featured image to use as background.', 'csismag' ),
					'priority'    => 42,
				)
			);
			...
			$wp_customize->add_control(
				...
				array(
					...
					'label'       => __( 'Fixed Background Image', 'csismag' ),
					'description' => __( 'Creates a parallax effect when the visitor scrolls.', 'csismag' ),
				)
			);
			...
			$wp_customize->add_setting(
				...
				array(
					'default'           => csismag_get_color_for_area( 'content', 'accent' ),
					'sanitize_callback' => 'sanitize_hex_color',
				)
			);
			...
			$wp_customize->add_control(
				new WP_Customize_Color_Control(
					...
					'cover_template_overlay_background_color',
					array(
						'label'       => __( 'Overlay Background Color', 'csismag' ),
						'description' => __( 'The color used for the overlay. Defaults to the accent color.', 'csismag' ),
						'section'     => 'cover_template_options',
					)
				)
			);
			...
			$wp_customize->add_control(
				new WP_Customize_Color_Control(
					...
					'cover_template_overlay_text_color',
					array(
						'label'       => __( 'Overlay Text Color', 'csismag' ),
						'description' => __( 'The color used for the text in the overlay.', 'csismag' ),
						'section'     => 'cover_template_options',
					)
				)
			);
			...
			$wp_customize->add_control(
				'cover_template_overlay_opacity',
				array(
					'label'       => __( 'Overlay Opacity', 'csismag' ),
					'description' => __( 'Make sure that the contrast is high enough so that the text is readable.', 'csismag' ),
					'section'     => 'cover_template_options',
					'type'        => 'range',
					'input_attrs' => csismag_customize_opacity_range(),
				)
			);
...
if ( ! function_exists( 'csismag_customize_partial_site_logo' ) ) {
	/**
	 * Render the site logo for the selective refresh partial.
	 *
	 * Doing it this way so we don't have issues with `render_callback`'s arguments.
	 */
	function csismag_customize_partial_site_logo() {
		csismag_site_logo();
	}
}
 ...
 */
function csismag_customize_opacity_range() {
	/**
	...
	return apply_filters(
		'csismag_customize_opacity_range',
		array(
			'min'  => 0,
			'max'  => 90,
			'step' => 5,
